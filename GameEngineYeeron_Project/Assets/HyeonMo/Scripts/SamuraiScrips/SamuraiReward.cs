...
public class SamuraiReward : MonoBehaviour {
    ...
    public void RewardRotateSword() {
        ...
        if (rotateSwordLevel < AbilityLevel.level_3) { //최대 검 3개 소환
    ...
    public void RewardSwordRotationSpeed() {
        if (swordRotationSpeedLevel < AbilityLevel.level_5) { //최대 회전 속도 2배 증가
    ...
    public void RewardInstallFlooring() {
        ...
        if (installFlooringLevel < AbilityLevel.level_5) { flooring.destroyTime += 1f; } //최대 5초가 늘어남
        else Debug.Log("이미 만렙인데 왜 그럼? 이건 장판 설치");
    }
}